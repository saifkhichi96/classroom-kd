...
def main():
    ...
    """build model"""
    if args.mixup > 0.0 or args.cutmix > 0 or args.cutmix_minmax is not None:
        loss_fn = SoftTargetCrossEntropy()
    elif args.smoothing == 0.0:
        loss_fn = nn.CrossEntropyLoss().cuda()
    else:
        loss_fn = CrossEntropyLabelSmooth(
            num_classes=args.num_classes, epsilon=args.smoothing
        ).cuda()
    val_loss_fn = loss_fn
    ...
    if args.kd != "":
            ...
            if len(args.teacher_ckpt):
                teacher_models = nn.ModuleList(
                    [
                        build_model(
                            args,
                            args.teacher_model[i],
                            args.teacher_pretrained,
                            args.teacher_ckpt[i],
                        )
                        for i in range(len(args.teacher_model))
                    ]
                )
            else:
                teacher_models = nn.ModuleList(
                    [
                        build_model(
                            args, args.teacher_model[i], args.teacher_pretrained
                        )
                        for i in range(len(args.teacher_model))
                    ]
                )
        ...
        if len(args.peer_ckpt):
            peer_models = nn.ModuleList(
                [
                    build_model(
                        args,
                        args.peer_model[i],
                        args.peer_pretrained,
                        args.peer_ckpt[i],
                    )
                    for i in range(len(args.peer_model))
                ]
            )
        else:
            peer_models = nn.ModuleList(
                [
                    build_model(args, args.peer_model[i], args.peer_pretrained)
                    for i in range(len(args.peer_model))
                ]
            )
        for i, teacher_model in enumerate(teacher_models):
            ...
            test_metrics = validate(
                ...
                val_loader,
                val_loss_fn,
                log_suffix=" (teacher){i}",
            )
            logger.info(
                f"Top-1 accuracy of teacher model {i}: {test_metrics['top1']:.2f}"
            )
        ...
        for i, peer_model in enumerate(peer_models):
            ...
            peer_model.cuda()
            test_metrics = validate(
                args, 0, peer_model, val_loader, val_loss_fn, log_suffix=" (peer){i}"
            )
        ...
        if args.ask or args.no_ask:
            ...
            from lib.models.losses.ask_kd_loss import AskKDLoss
            ...
            if args.no_mentor:
                loss_fn = AskKDLoss(
                    ...
                    loss_fn,
                    kd_method=args.kd,
                    ori_loss_weight=args.ori_loss_weight,
                    kd_loss_weight=args.kd_loss_weight,
                    peer_loss_weight=args.peer_loss_weight,
    ...
    if args.dyrep:
        ...
        dyrep = DyRep(
            ...
            recal_bn_fn=lambda m: recal_bn(
                model.module, train_loader, args.dyrep_recal_bn_iters, m
            ),
            filter_bias_and_bn=not args.opt_no_filter,
        )
    ...
    ckpt_manager = CheckpointManager(
        ...
        rank=args.rank,
        additions={"scaler": loss_scaler, "dyrep": dyrep},
    )
    ...
    if args.resume:
        ...
        if args.dyrep:
            model = DDP(
                model.module, device_ids=[args.local_rank], find_unused_parameters=True
            )
        logger.info(
            f"Resume ckpt {args.resume} done, start training from epoch {start_epoch}"
        )
    else:
        start_epoch = 0
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.drop_path_rate > 0.0 and args.drop_path_strategy == "linear":
        ...
        if model_ema is not None:
            test_metrics = validate(
                args, epoch, model_ema.module, val_loader, loss_fn, log_suffix="(EMA)"
            )
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.kd:
            ...
            # decay_factor not used in paper
