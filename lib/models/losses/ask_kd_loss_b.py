...
class AskKDLoss():
    ...
    def __call__(self, x, targets,decay_factor):
        ...
        with torch.no_grad():
            t_logits = [teacher(x).detach() for teacher in self.teachers]
            p_logits = [peer(x).detach() for peer in self.peers]

        # Compute original loss of the student
        logits_ori = self.student(x)
        logits = logits_ori.detach()
        ori_loss = self.ori_loss(logits_ori, targets)
        num_classes = logits.size(-1)
        one_hot_labels = nn.functional.one_hot(targets, num_classes=num_classes).float()
        target_mask = torch.sum(one_hot_labels, dim=0).bool()
        student_probs = nn.functional.softmax(logits, dim=-1)
        student_pred_correct = torch.masked_select(student_probs,target_mask)
        self.weights['student'] = torch.sum(student_pred_correct,dim=0)/student_probs.size(0)

        for i,peer_logits in enumerate(p_logits):
            peer_probs = nn.functional.softmax(peer_logits, dim=-1)
            peer_pred_correct = torch.masked_select(peer_probs,target_mask)
            self.weights[i] = torch.sum(peer_pred_correct,dim=0)/peer_probs.size(0)
        
        teacher_probs = nn.functional.softmax(t_logits[0], dim=-1)
        teacher_pred_correct = torch.masked_select(teacher_probs,target_mask)
        self.weights['teacher'] = torch.sum(teacher_pred_correct,dim=0)/teacher_probs.size(0)

        sorted_weights = dict(sorted(self.weights.items(), key=lambda item: item[1]))

        # Update the values with their corresponding ranks
        self.weights = {key: [(i + 1)/10,value] for i, (key, value) in enumerate(sorted_weights.items())}
        kd_loss = 0
        peer_loss = 0
        if(len(self.teachers)):
            for i, (teacher, t_logit) in enumerate(zip(self.teachers, t_logits)):
                for tm, sm in zip(self.teacher_modules[i], self.student_modules):
                    tau_scale = (abs(self.weights['teacher'][1]-self.weights['student'][1]))
                    kd_loss_ = self.kd_loss_t(self._student_out[sm], self._teacher_out[i][tm],tau_scale=tau_scale)
                    if(self.weights['teacher'][0]>self.weights['student'][0]):
                        kd_loss_ *= self.weights['teacher'][1]
                    else:
                        # print(f"do not ask peer {i}. Confidence is lower than student")
                        kd_loss_ *= 0.0
                    if self._iter % 50 == 0:
                        logger.info(f'[{tm}-{sm}]t{i} KD ({self.kd_method}) loss: {kd_loss_.item():.4f} teacher_tau: {tau_scale*12:.3f}')
                    kd_loss += kd_loss_
        if(len(self.peers)):
            for i, (peer, p_logit) in enumerate(zip(self.peers, p_logits)):
                for pm, sm in zip(self.peer_modules[i], self.student_modules):
                    tau_scale = (abs(self.weights[i][1]-self.weights['student'][1]))
                    peer_loss_ = self.kd_loss_p(self._student_out[sm], self._peer_out[i][pm],tau_scale=tau_scale)
                    if(self.weights[i][0]>self.weights['student'][0]):
                        peer_loss_*= self.weights[i][0]
                    else:
                        # print(f"do not ask peer {i}. Confidence is lower than student")
                        peer_loss_ *= 0.0

                    if self._iter % 50 == 0:
                        logger.info(f'[{pm}-{sm}]p{i} KD ({self.kd_method}) loss: {peer_loss_.item():.4f} peer_tau: {tau_scale*12:.3f}')
                    peer_loss += peer_loss_
        if self._iter % 50 == 0:
            logger.info(f'weights: {self.weights}')
        student_pred_correct_avg = self.weights['student'][1]
