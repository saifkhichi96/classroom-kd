...
def build_model(args, model_name, pretrained=False, pretrained_ckpt=""):
    if model_name.lower() == "nas_model":
        ...
        edgenn_model = EdgeNNModel(
            model, loss_fn=None, pruning=True, input_shape=args.input_shape
        )
        logger.info(edgenn_model.graph)
        edgenn_model.fold_dynamic_nn(
            channel_settings["choices"],
            channel_settings["bins"],
            channel_settings["min_bins"],
        )
        logger.info(model)
        ...
        model = timm.create_model(
            model_name[5:], pretrained=pretrained, drop_path_rate=args.drop_path_rate
        )

    elif model_name.startswith("cifar_efficient"):
        from .cifar import model_dict

        model_name = model_name[6:]
        model = model_dict[model_name](num_classes=args.num_classes, pretrained=False)
    ...
    if pretrained and pretrained_ckpt != "":
        ...
        if "net" in ckpt:
            acc = ckpt["acc"]
            logger.info(f"ckpt acc before loading {acc}")
            ckpt = ckpt["net"]
